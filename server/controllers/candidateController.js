    ...
    await candidate
      .findOne({
        ...
        include: [
          ...
          {
            model: requirement,
            attributes: ["id", "requirementName", "uniqueId", "experience"],
            include: [
              {
                model: client,
...
exports.checkMobileExist=async(req,res)=>{
  const roleName=req.roleName;
  if(roleName=="FREELANCER"||roleName=="SUBVENDOR"){
  candidateDetails.findOne({where:{mobile:"91"+req.body.mobile,mainId:req.mainId}}).then(data=>{
